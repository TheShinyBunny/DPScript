...
public class Parser {
    ...
    private List<String> bossbars = new ArrayList<>();
    ...
    public Parser(File file) {
        this.file = file;
        this.originalCode = getCode(file);
        this.tokens = new TokenIterator(Tokenizer.tokenize(file,String.join("\n",originalCode)),this::compilationError);
    ...
    private void parse() {
        ...
        if (!errors.isEmpty()) {
            ...
            loadCommands.forEach(System.out::println);
            System.out.println(">> TICK:");
            tickCommands.forEach(System.out::println);
    ...
    private void parseGlobal() {
        ...
        switch (t.getValue()) {
            ...
            case "bossbar":
                ...
                loadCommands.add("bossbar add " + id + " " + displayName);
                bossbars.add(id);
                break;
            case "const": {
            ...
            case "int": {
                ...
                objectives.add(name);
                loadCommands.add("scoreboard objectives add " + name + " dummy");
                break;
            }
            default:
                tokens.pushBack();
            ...
            default:
                if (tokens.skipAll("(",")")) {
                    if (functions.containsKey(token)) {
                        ...
                        compilationError(ErrorType.UNKNOWN,"function " + token);
                    }
                } else if (globals.contains(token)) {
                    list.addAll(selectors.parseScoreOperators(getVariableAccess(token)));
                } else if (bossbars.contains(token)) {
    ...
    public void createConstant(String name, int value) {
        ...
        loadCommands.add("scoreboard players set " + name + " Constants " + value);
    }
    ...
    private void ensureGlobal() {
        ...
        createdGlobal = true;
        loadCommands.add("scoreboard objectives add Global dummy");
    }
